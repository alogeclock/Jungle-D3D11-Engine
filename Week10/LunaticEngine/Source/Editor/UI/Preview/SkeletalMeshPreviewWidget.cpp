...
bool FSkeletalMeshPreviewWidget::Initialize(UEditorEngine* InEngine, ID3D11Device* InDevice, FWindowsWindow* InWindow)
{
	if (bInitialized)
	{
		return true;
	...
	if (!PreviewClient)
	{
		...
		delete Viewport;
		Viewport = nullptr;
		FNotificationManager::Get().AddNotification("Failed to create SkeletalMesh preview client.", ENotificationType::Error, 5.0f);
		return false;
	}
...
{
	if (!bInitialized && !Viewport && !GetPreviewViewportClient())
	{
		return;
	}
