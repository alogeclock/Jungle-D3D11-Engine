...
local PlayerControllerConfig = {
    -- forward_speed: 플레이어가 매 프레임 자동 전진하는 속도입니다.
    forward_speed = 10.0,

    -- max_move_step: overlap 누락을 줄이기 위해 전진 이동을 쪼개는 최대 거리입니다.
    max_move_step = 0.25,

    -- lane_*: 레인 개수, 간격, 레인 이동 보간 속도입니다.
    lane_width = 4.0,
    lane_count = 3,
    lane_change_speed = 12.0,

    -- barrel_roll_*: 레인 변경 시 포드 롤 연출 설정입니다.
    barrel_roll_enabled = true,
    barrel_roll_degrees = 360.0,
    barrel_roll_duration = 0.34,

    -- gravity / jump_power: 점프와 낙하에 쓰는 기본 물리값입니다.
    gravity = -25.0,
    jump_power = 10.0,

    -- fall_dead_z: 이 높이 아래로 떨어지면 낙사 처리합니다.
    fall_dead_z = -10.0,

    -- ground_* / skin_width: 바닥 감지와 snap 보정값입니다.
    ...
    -- coach_window_textures: PlayerController가 코치 대화창을 직접 갱신할 때 쓰는 초상화 창입니다.
