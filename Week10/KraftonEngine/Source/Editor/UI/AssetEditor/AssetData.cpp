#include "Editor/UI/AssetEditor/AssetData.h"

#include "Editor/UI/AssetEditor/AssetFileSerializer.h"
...
void UAssetData::Serialize(FArchive& Ar)
{
...
float FAssetBezierCurve::Evaluate(float NormalizedTime) const
{
	...
	// x를 기준으로 y를 얻기 위해 binary search로 Bezier parameter t를 근사한다.
	float Low = 0.0f;
	float High = 1.0f;
