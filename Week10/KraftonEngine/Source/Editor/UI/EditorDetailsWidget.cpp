...
void FEditorDetailsWidget::RenderHeader(AActor* PrimaryActor, const TArray<AActor*>& SelectedActors)
{
	...
	if (SelectionCount <= 1)
	{
		const float InputWidth = (std::max)(120.0f, ImGui::GetContentRegionAvail().x - 84.0f);
		if (bEditingActorName)
		{
	...
	ImGui::SameLine((std::max)(0.0f, ImGui::GetWindowContentRegionMax().x - 76.0f));
...
void FEditorDetailsWidget::RenderAddComponentButton(AActor* Actor)
{
	...
	if (ImGui::Button("+ Add", ImVec2(72.0f, 0.0f)))
	{
