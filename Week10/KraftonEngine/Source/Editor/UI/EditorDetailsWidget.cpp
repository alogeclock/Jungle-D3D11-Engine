...
namespace
{
	...
	bool DrawHeaderIconButton(const char* Id, const char* IconKey, const char* FallbackLabel, const char* Tooltip, const ImVec2& Size, ImU32 Tint = IM_COL32_WHITE)
	{
		const bool bClicked = ImGui::Button(Id, Size);
		const ImVec2 Min = ImGui::GetItemRectMin();
		const ImVec2 Max = ImGui::GetItemRectMax();
	...
	bool DrawIconLabelButton(const char* Id, const char* IconKey, const char* FallbackLabel, const char* Tooltip, const ImVec2& Size, ImU32 Tint = IM_COL32_WHITE)
	{
		const bool bClicked = ImGui::Button(Id, Size);
		const ImVec2 Min = ImGui::GetItemRectMin();
		const ImVec2 Max = ImGui::GetItemRectMax();
...
{
	if (RawName == "bTickEnable") return "Tick Enabled";
	if (RawName == "bEditorOnly") return "Editor Only";
	if (RawName == "Is Collidable") return "Collision Enabled";
	if (RawName == "Generates Overlap Event") return "Generate Overlap Events";
...
bool FEditorDetailsWidget::DrawColoredFloat3(const char* Label, float Values[3], float Speed)
{
	...
	for (int32 Axis = 0; Axis < 3; ++Axis)
	{
		...
		ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImVec4(0.08f, 0.08f, 0.10f, 1.0f));
...
bool FEditorDetailsWidget::DrawColoredFloat4(const char* Label, float Values[4], float Speed)
{
	return ImGui::DragFloat4(Label, Values, Speed);
}
...
void FEditorPropertyWidget::Render(float DeltaTime)
{
	...
	if (ScrollHeight < MinDetailsHeight) ScrollHeight = MinDetailsHeight;
...
void FEditorDetailsWidget::RenderDetailsFilterBar(const TArray<const char*>& AvailableSections)
{
	...
	for (int32 SectionIndex = -1; SectionIndex < static_cast<int32>(AvailableSections.size()); ++SectionIndex)
	{
		...
		ImGui::PushStyleColor(ImGuiCol_Button, bActive ? ImVec4(0.06f, 0.33f, 0.75f, 1.0f) : ImVec4(0.18f, 0.18f, 0.19f, 1.0f));
		ImGui::PushStyleColor(ImGuiCol_ButtonHovered, bActive ? ImVec4(0.09f, 0.39f, 0.84f, 1.0f) : ImVec4(0.23f, 0.23f, 0.24f, 1.0f));
		ImGui::PushStyleColor(ImGuiCol_ButtonActive, bActive ? ImVec4(0.05f, 0.28f, 0.67f, 1.0f) : ImVec4(0.16f, 0.16f, 0.17f, 1.0f));
		ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(0.02f, 0.02f, 0.02f, 1.0f));
...
void FEditorDetailsWidget::RenderAddComponentButton(AActor* Actor)
{
	...
	TArray<FComponentClassGroup> Groups;
	AddComponentClassGroup(Groups, "LIGHT", ULightComponentBase::StaticClass());
	AddComponentClassGroup(Groups, "MOVEMENT", UMovementComponent::StaticClass());
	AddComponentClassGroup(Groups, "EFFECTS", UBillboardComponent::StaticClass());
	AddComponentClassGroup(Groups, "PRIMITIVE", UPrimitiveComponent::StaticClass());
	AddComponentClassGroup(Groups, "SCENE", USceneComponent::StaticClass());
	AddComponentClassGroup(Groups, "OTHER", nullptr);
	...
	for (UClass* Cls : UClass::GetAllClasses())
	{
		...
		{
			UClass* AnchorClass = FindComponentClassGroupAnchor(Cls, Groups);
			FComponentClassGroup* TargetGroup = nullptr;
			for (FComponentClassGroup& Group : Groups)
			{
				if ((AnchorClass && Group.AnchorClass == AnchorClass)
					|| (!AnchorClass && Group.AnchorClass == nullptr))
				{
					TargetGroup = &Group;
					break;
				}
			...
			if (TargetGroup)
			{
				TargetGroup->Classes.push_back(Cls);
			}
		}
	}
...
void FEditorPropertyWidget::RenderActorProperties(AActor* PrimaryActor, const TArray<AActor*>& SelectedActors)
{
	...
	const bool bMatchesTransform = DetailSearchBuffer[0] == '\0'
		|| ContainsCaseInsensitive("Transform", SearchQuery)
...
void FEditorPropertyWidget::RenderComponentTree(AActor* Actor, float Height)
{
	...
	{
		const bool bCanDelete = SelectedComponent->CanDeleteFromDetails();
		if (!bCanDelete)
		{
			ImGui::BeginDisabled();
		}
		ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.35f, 0.14f, 0.14f, 1.0f));
		ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.44f, 0.18f, 0.18f, 1.0f));
		ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0.26f, 0.10f, 0.10f, 1.0f));
		const float DeleteButtonWidth = 112.0f;
		ImGui::SetCursorPosX((std::max)(ImGui::GetCursorPosX(), ImGui::GetWindowContentRegionMax().x - DeleteButtonWidth));
		if (DrawIconLabelButton("##DeleteComponent", "Editor.Icon.Delete", "Delete", "Delete Component", ImVec2(DeleteButtonWidth, 0.0f), IM_COL32(255, 225, 225, 255)) && bCanDelete)
		{
			EditorEngine->BeginTrackedSceneChange();
...
void FEditorPropertyWidget::RenderComponentProperties(AActor* Actor, const TArray<AActor*>& SelectedActors)
{
	...
	TArray<const char*> AvailableSections;
	if (!TransformIndices.empty()) AvailableSections.push_back("Transform");
	if (!StaticMeshIndices.empty()) AvailableSections.push_back("Static Mesh");
	if (!MaterialIndices.empty() || (SelectedComponent && SelectedComponent->IsA<UStaticMeshComponent>())) AvailableSections.push_back("Materials");
	if (!VisibilityIndices.empty()) AvailableSections.push_back("Visibility");
	if (!BehaviorIndices.empty()) AvailableSections.push_back("Behavior");
	if (!DefaultIndices.empty()) AvailableSections.push_back("Default");
	RenderDetailsFilterBar(AvailableSections);
	...
	if (ShouldDisplaySection("Materials", Props, MaterialIndices))
	{
		...
		&& SelectedComponent
		&& SelectedComponent->IsA<UStaticMeshComponent>()
...
void FEditorDetailsWidget::RenderPropertySection(const char* SectionName, TArray<FPropertyDescriptor>& Props, const TArray<int32>& Indices, const TArray<AActor*>& SelectedActors, bool& bAnyChanged)
{
	if (Indices.empty())
	{
		return;
	...
	for (AActor* Actor : SelectedActors)
	{
		...
		for (UActorComponent* Comp : Actor->GetComponents())
		{
			...
			for (const auto& DstProp : DstProps)
			{
				...
				case EPropertyType::TextureSlot:    *static_cast<FTextureSlot*>(DstProp.ValuePtr) = *static_cast<FTextureSlot*>(SrcProp->ValuePtr); break;
...
bool FEditorPropertyWidget::RenderPropertyWidget(TArray<FPropertyDescriptor>& Props, int32& Index)
{
	...
	case EPropertyType::MaterialSlot:
	{
		...
		const float ReservedPreviewWidth = (CurrentPreviewTexture && CurrentPreviewTexture->GetSRV())
			? (PreviewImageSize + PreviewSpacing)
			: 0.0f;
