...
기본 브랜치 `main`은 `Week01/`~`Week14/`의 현재 파일을 하나의 스냅샷으로 제공합니다. 통합된 전체 커밋 이력은 `archive/full-history`에 보존했으며, 각 프로젝트의 기본 브랜치가 아닌 브랜치도 `archive/weekXX/bNNN/<original-name>` 형식으로 보존했습니다. `bNNN`은 Windows에서 대소문자 또는 경로가 충돌하는 Git 브랜치를 함께 보존하기 위한 고유 식별자입니다.
...
git log --oneline origin/archive/full-history -- Week05
